...
namespace Marmi
{
    public partial class Form1 : Form
    {
        ...
        private void SetKeyConfig2()
        {
            ...
            setkey(App.Config.Keys.Key_Nexthalf2, async () => { await SetViewPageAsync(++App.g_pi.NowViewPage); });
            setkey(App.Config.Keys.Key_Prevhalf1, async () => { await SetViewPageAsync(--App.g_pi.NowViewPage); });
            setkey(App.Config.Keys.Key_Prevhalf2, async () => { await SetViewPageAsync(--App.g_pi.NowViewPage); });

            //先頭最終ページ
            setkey(App.Config.Keys.Key_Toppage1, async () => { await SetViewPageAsync(0); });
            setkey(App.Config.Keys.Key_Toppage2, async () => { await SetViewPageAsync(0); });
            setkey(App.Config.Keys.Key_Lastpage1, async () => { await SetViewPageAsync(App.g_pi.Items.Count - 1); });
            setkey(App.Config.Keys.Key_Lastpage2, async () => { await SetViewPageAsync(App.g_pi.Items.Count - 1); });

            //ブックマーク
            setkey(App.Config.Keys.Key_Bookmark1, ToggleBookmark);
            setkey(App.Config.Keys.Key_Bookmark2, ToggleBookmark);

            //フルスクリーン
            setkey(App.Config.Keys.Key_Fullscreen1, ToggleFullScreen);
            setkey(App.Config.Keys.Key_Fullscreen2, ToggleFullScreen);
            //２画面モード切替
            setkey(App.Config.Keys.Key_Dualview1, async () => { await SetDualViewModeAsync(!App.Config.DualView); });
            setkey(App.Config.Keys.Key_Dualview2, async () => { await SetDualViewModeAsync(!App.Config.DualView); });
            // ゴミ箱
            setkey(App.Config.Keys.Key_Recycle1, async () => { await RecycleBinNowPageAsync(); });
            setkey(App.Config.Keys.Key_Recycle2, async () => { await RecycleBinNowPageAsync(); });
            //表示モード
            setkey(App.Config.Keys.Key_ViewRatio1, ToggleFitScreen);
            setkey(App.Config.Keys.Key_ViewRatio2, ToggleFitScreen);
            // 終了 ver1.77
            ...
            setkey(App.Config.Keys.Key_Rotate2, () => PicPanel.Rotate());
