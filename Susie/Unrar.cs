...
namespace Marmi
{
    ...
    public class Unrar : IDisposable
    {
        ...
        {
            List = 0,   // Open archive for listing contents only
            Extract = 1 // Open archive for testing or extracting contents
        }
        ...
        public Unrar()
        {
            ...
            LoadUnrarDll("unrar.dll");
        }

        private bool LoadUnrarDll(string unrardll)
        {
            try
            {
                dllHandle = LoadLibrary(unrardll);
                if (dllHandle != IntPtr.Zero)
                {
                    IntPtr ptr = GetProcAddress(dllHandle, "RAROpenArchive");
                    RAROpenArchive = GetProcDelegate<_RAROpenArchive>(dllHandle, "RAROpenArchive");
                    RAROpenArchiveEx = GetProcDelegate<_RAROpenArchiveEx>(dllHandle, "RAROpenArchiveEx");
                    RARCloseArchive = GetProcDelegate<_RARCloseArchive>(dllHandle, "RARCloseArchive");
                    RARReadHeader = GetProcDelegate<_RARReadHeader>(dllHandle, "RARReadHeader");
                    RARReadHeaderEx = GetProcDelegate<_RARReadHeaderEx>(dllHandle, "RARReadHeaderEx");
                    RARProcessFile = GetProcDelegate<_RARProcessFile>(dllHandle, "RARProcessFile");
                    RARSetCallback = GetProcDelegate<_RARSetCallback>(dllHandle, "RARSetCallback");
                    RARSetPassword = GetProcDelegate<_RARSetPassword>(dllHandle, "RARSetPassword");
                    return true;
                }
                else
                    return false;
        ...
        /// <summary>
        /// dllからGetProcAddressし、Delegateを返すメソッド
        /// </summary>
        /// <typeparam name="T">Delegate型情報</typeparam>
        /// <param name="dllHandle">LoadLibrary(unrar.dll)のハンドル</param>
        /// <param name="method">読み出すメソッド/関数名</param>
        /// <returns>指定したDelegate型の関数ポインタ</returns>
        public T GetProcDelegate<T>(IntPtr dllHandle, string method) where T : class
        {
            IntPtr methodHandle = GetProcAddress(dllHandle, method);
        ...
        public bool ReadHeader()
        {
            ...
            int result = RARReadHeaderEx(this.archiveHandle, ref this.header);
        ...
        public void Skip()
        {
            ...
            int result = RARProcessFile(this.archiveHandle, (int)Operation.Skip, string.Empty, string.Empty);
        ...
        public void Test()
        {
            ...
            int result = RARProcessFile(this.archiveHandle, (int)Operation.Test, string.Empty, string.Empty);
        ...
        private void Extract(string destinationPath, string destinationName)
        {
            ...
            int result = RARProcessFile(this.archiveHandle, (int)Operation.Extract, destinationPath, destinationName);
